	...
	static UnicodeBracketedExpression getRegexConcat() {
		final UBrexConcat simple = UBrexConcat.build( //
				...
				new UBrexOptional(UBrexConcat.build( //
						...
						new UBrexNamed("ARROW_STYLE1",
								new UBrexLeaf("〇?〘 [" + CommandLinkElement.UBREX_LINE_STYLE + "] 〙")), //
						new UBrexNamed("ARROW_DIRECTION",
								new UBrexLeaf("〇?【 *┇left┇right┇up┇down┇l〇?e┇r〇?i┇u〇?p┇d〇?o】")), //
						new UBrexNamed("ARROW_STYLE2",
								new UBrexLeaf("〇?〘 [" + CommandLinkElement.UBREX_LINE_STYLE + "] 〙")), //
				...
				new UBrexOr(//
						new UBrexLeaf("if 〇*〴s 〴g  〶$IF1=〇*〴G 〴g 〇*〴s 〇?〘as 〇+〴s 〶$ASIF1=〇+「〴an_.」 〇+〴s 〙  〇?〘then〙"), //
						new UBrexLeaf("if 〇+〴s 〶$IF2=〇l+〴. 〇?〘then〙 ") //
				));
